defmodule Language.Interpreter do
  ...
  def reduce_expr(expr, state \\ %Environment{})
  ...
  def reduce_expr({:if, {condition, when_true, when_false}}, state) do
    case reduce_expr(condition, state) do
      {:ok, true} -> reduce_expr(when_true, state)
      {:ok, false} -> reduce_expr(when_false, state)
      {:ok, _} -> {:error, "conditional expressions given to `if` must reduce to Booleans."}
      error -> error
    end
  end
  ...
  def reduce_expr({:=, args}, state = %Environment{}) do
    with {:ok, args} <- reduce_args(args, state) do
      if length(args) >= 2 do
  ...
  def reduce_expr({:event, event, _args}, _state) do
    {:error, "custom events like `#{event}` have not yet been implemented"}
  end

  defp reduce_args(args, state) do
    with {:ok, args} <- resolve_variables(state, args),
    ...
    # with :none <- find_expr_errors(values) do
    #   {:ok, Enum.map(values, fn {:ok, x} -> x end)}
    # end
  end
  ...
  @spec resolve_variables(Environment.t(), [any]) ::
          {:ok, [Language.variable()]} | {:error, String.t()}
  def resolve_variables(variables, args) do
    with args when is_list(args) <- do_resolve_variables(variables, args) do
      {:ok, Enum.reverse(args)}
    end
  end

  defp do_resolve_variables(variables, args) do
    Enum.reduce_while(args, [], fn arg, acc ->
      case resolve_var(arg, variables) do
        {:ok, var} -> {:cont, [var | acc]}
  ...
  defp resolve_var({:variable, {player, var_name}}, vars) do
    with {:ok, player_vars} <- Environment.get(vars, var_name),
         {:ok, var} <- Map.fetch(player_vars, player) do
      {:ok, var}
    else
      :not_found -> var_not_found(var_name)
      :error -> {:error, "role `#{player}` does not exist"}
    end
  end

  defp resolve_var({:variable, var_name}, vars) do
    case Environment.get(vars, var_name) do
      :not_found -> var_not_found(var_name)
      found -> found
    end
  end

  defp resolve_var(normal_value, _vars) do
    {:ok, normal_value}
  end

  defp var_not_found(variable) do
